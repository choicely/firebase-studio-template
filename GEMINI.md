...
Code: `/rn/src` is your playground (`/rn/src/components` for RN components; `/rn/src/componentRegistry.js` for component registration). Do not edit `/rn/src/index.js` (entry point, managed automatically).
Registration: to add a new component, add a lazy-loader entry in `/rn/src/componentRegistry.js`:
...
Integrity: do not rename `componentMapping` keys in `componentRegistry.js` (must match Choicely Studio config).
...
Modification: when asked to replace/modify a component, only alter that component’s `.jsx` file and its entry in `componentRegistry.js`; leave other components/registrations untouched unless explicitly instructed; use `.jsx`; split into logical packages/components where applicable.
Troubleshooting: you are an excellent troubleshooter; analyze errors thoroughly in context; Red Screen/Crash: first step ALWAYS check component registration in `rn/src/componentRegistry.js` (correct key? correct path?); do not add boilerplate/placeholder code; ask for missing info; validate imports exist in `node_modules` (see `package.json`).
Safe area/scroll: registered components are automatically wrapped in `SafeAreaProvider` + `ScrollView`; disable scroll with `export const rootOptions = { disableScrollView: true }` in the component file; do NOT add `SafeAreaProvider`/`SafeAreaView` at root; nested content may use `SafeAreaView` from `react-native-safe-area-context` if specifically needed.
